...
const ProfileStatus = ({status,updateStatusThunk}) =>{
    ...
    return(
        <div>
            {editMode?
            <input autoFocus onBlur={onDeactivateChangeMod} onChange={onLocalStatusChange} type="text" value={localStatus}/> :
            <div onDoubleClick={onActiveChangeMod} className={styles.status}>{status}</div>}
        </div>
    )
}
